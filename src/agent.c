...
void agent_set_remote_description(Agent *agent, char *description)
{
    ...
    */
    ...
    LOGI("remote candidates count: %d", agent->remote_candidates_count);
...
int agent_connectivity_check(Agent *agent)
{
    ...
    if (agent->nominated_pair->state == ICE_CANDIDATE_STATE_WAITING)
    {
        ...
        LOGI("send binding request to remote ip: %d.%d.%d.%d, port: %d", agent->nominated_pair->remote->addr.ipv4[0], agent->nominated_pair->remote->addr.ipv4[1], agent->nominated_pair->remote->addr.ipv4[2], agent->nominated_pair->remote->addr.ipv4[3], agent->nominated_pair->remote->addr.port);
...
void agent_select_candidate_pair(Agent *agent)
{
    ...
    for (i = 0; i < agent->candidate_pairs_num; i++)
    {
        ...
        else if (agent->candidate_pairs[i].state == ICE_CANDIDATE_STATE_INPROGRESS && (t2 - agent->candidate_pairs[i].nominated_time) > 2)
        {
